...
- **An oam host older than the floor no longer runs the CLI itself.** When a host ran `oam run bin/ctxlint.mjs` with an old oam and discovery found nothing newer, ctxlint ran on that old oam -- and below 0.9.0 oam passes `child_process` arguments through a shell, while ctxlint's git-backed checks spawn `git`. An old host now hands off to the newest usable oam, or to Node on `PATH`, or exits 1 with a remedy when there is neither. The handoff pipes stdio, because an old oam treats `stdio: 'inherit'` as `'pipe'`: on `@yawlabs/aws-mcp`'s identical launcher an inherited handoff from a real oam 0.8.2 host never answered the MCP handshake. A Node host still inherits stdio, and the child's exit code is still passed through. If the chosen oam passes its version check but then cannot be spawned (deleted or replaced in between), the handoff still falls back to Node: piping, signal forwarding and the exit mirror all wait for the child's `spawn` event, because a failed spawn still emits `close` (with the negative errno) and the piped path exits on `close`.
...
- The launcher's new `pickNewest()` choice has unit tests beside `runtimePlan()`'s, and the real bin is run under a preloaded `process.versions.oam` with no real oam in reach: a missing `OAM_BIN` is named, `CTXLINT_RUNTIME=oam` exits 1, a below-floor host hands off to a Node child (or exits 1 with no Node on `PATH`), `CTXLINT_RUNTIME=node`/`NODE` hands off from a supported oam host, and a below-floor host whose chosen oam fails to spawn (a preload points the first `spawn` at a missing path) still runs the CLI through the Node fallback. Reverting to first-found selection, letting a below-floor host run in-process, dropping the floor back to 0.9.0, or letting the exit mirror act on a child that never spawned each turns the suite red.
