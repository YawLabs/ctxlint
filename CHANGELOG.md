...
- **The MCP SDK devDependency moves from `^1.29.0` to `^1.30.0`, and `pnpm audit` goes from 28 findings (10 high, 17 moderate, 1 low) to 0.** Every finding was development-scope, and all but the direct `vitest` devDependency were transitive, through the SDK or the build/test toolchain: `fast-uri` 3.1.2 -> 3.1.7, `hono` 4.12.25 -> 4.13.7, `@hono/node-server` 1.19.14 -> 1.19.17, `ip-address` 10.2.0 -> 10.7.0, `qs` 6.15.2 -> 6.16.0, `postcss` 8.5.19 -> 8.5.28, `nanoid` 3.3.16 -> 3.3.19, the lockfile's `brace-expansion` 5.0.7 -> 5.0.12, and `vitest` / `@vitest/coverage-v8` / `@vitest/mocker` 4.1.9 -> 4.1.11 (Dependabot auto-dismissed seven of its alerts -- `hono` GHSA-8j4g and GHSA-g6gw, `qs` x2, `nanoid` and `brace-expansion` x2 -- so those showed up only in `pnpm audit`). The `pnpm.overrides` floors are raised to each advisory's first patched version (`hono` `^4.13.5`, `@hono/node-server` `^1.19.15`, `fast-uri` `^3.1.6`, `ip-address` `^10.3.1`, `qs` `^6.16.0`, `postcss` `^8.5.23`, `brace-expansion` `^5.0.9`) so a lockfile refresh cannot slide back into the advised ranges; with pnpm, raising the SDK floor alone re-resolves nothing transitive. Of the alerted packages, only `fast-uri` (via the SDK's `ajv` validator) was inlined into the published 0.27.1 `dist/index.js`: that bundle carried 3.1.2, inside the advised range of six high-severity host-confusion and SSRF advisories, and the bundle now carries 3.1.7. ajv resolves only ctxlint's own tool schemas, so the vulnerable path was never reachable from user input, but the shipped copy is updated regardless. The lockfile's `brace-expansion` is inlined as well now, through the `glob/raw` change in the next entry; none of the others is in the bundle.
- **`dist/index.js` now bundles the lockfile's patched `brace-expansion` instead of the pre-5.0.6 copy vendored inside `glob`.** `glob`'s default entry point (since 13.0.2) is a self-contained `index.min.js` with its own `minimatch` and `brace-expansion` inlined, and the 0.27.1 bundle shipped that copy: its numeric-range loop has no `max` cap, so it is open to GHSA-jxxr-4gwj-5jf2 and the later `brace-expansion` DoS advisories. No `pnpm.overrides` floor can reach code vendored that way, so the 0.15.1 entry below was not accurate when it said the `>=5.0.6` override fixed "the bundled copy": the override only moved the lockfile's copy, which the bundle never used. `src/core/scanner.ts` and `src/core/checks/paths.ts` now import from `glob/raw`, `glob`'s unbundled entry that depends on `minimatch` as a normal package, so esbuild inlines the lockfile's `minimatch` 10.2.5 and `brace-expansion` 5.0.12, with the `max`, `maxLength`, `maxDepth` and `maxRewrites` caps. Same `glob` version and API; `dist/index.js` grows by about 107 KB against published 0.27.1, about 94 KB of it from this change, because the unminified modules replace `glob`'s minified one. Neither Dependabot nor `pnpm audit` sees vendored code, so this was found by reading the published bundle.
- `fixtures/masked-exit-status/package.json` moves its `vitest` range from `^2.0.0` to `^4.1.11`, closing the one Dependabot alert (GHSA-5xrq-8626-4rwp) raised against a manifest that is never installed: the fixture is linter input for the `commands/exit-status-masked` test, has no lockfile, is not a workspace, and is excluded from the npm tarball. The test reads only dependency key names, so the value is inert.
- Pre-commit users pin the version through `.pre-commit-hooks.yaml` (`npx @yawlabs/ctxlint@<version>`) and need to bump `rev` to pick up the patched bundle. GitHub Action users on the default `version: latest`, `npx` users and MCP Registry clients get it on their next resolve.
...
- **The launcher always uses the newest oam, and the minimum is now the latest release, 0.15.2.** It used to take the FIRST oam binary it found and only then check its version, so a stale copy in an earlier location hid a current one: with oam 0.9.0 in `~/.oam/bin` and 0.15.2 on `PATH`, it ran 0.9.0. Every oam binary it can see (installed locations, then `PATH`, de-duplicated) is now asked for its version, and the newest at or above 0.15.2 wins; a tie keeps search order. Each `oam --version` probe is bounded at 5s, so a wedged binary on `PATH` cannot hang the launch.
- **An oam host older than the floor no longer runs the CLI itself.** When a host ran `oam run bin/ctxlint.mjs` with an older oam, ctxlint ran on it: directly when that oam cleared the old 0.9.0 floor, and below 0.9.0 whenever discovery found no usable oam or the one it found would not start. The floor is support policy -- only the latest oam is used and verified -- rather than a fix for an exposure: the CLI's only child process is `git`, started by simple-git through `spawn` with piped stdio, and on a real oam 0.8.2 `spawn` passes arguments through no shell (it is `execFile` that goes through one there, and ctxlint never calls it). An old host now hands off to the newest usable oam, or to Node on `PATH`, or exits 1 with a remedy when there is neither. The handoff pipes stdio, because oam before 0.9.0 treats `stdio: 'inherit'` as `'pipe'`: on `@yawlabs/aws-mcp`'s identical launcher an inherited handoff from a real oam 0.8.2 host never answered the MCP handshake. A Node host still inherits stdio, and the child's exit code is still passed through. If the chosen oam passes its version check but then cannot be spawned (deleted or replaced in between), the handoff still falls back to Node: piping, signal forwarding and the exit mirror all wait for the child's `spawn` event, because a failed spawn still emits `close` (with the negative errno) and the piped path exits on `close`.
- **A bad `OAM_BIN` is reported instead of ending discovery.** A path that does not exist (previously a silent drop to Node), an oam below the floor, or a binary that will not run is named on stderr, and discovery carries on to the installed locations and `PATH`.
- **`CTXLINT_RUNTIME=node` now always means Node.** Launched under `oam run`, it hands off to Node on `PATH` rather than staying on oam. The value stays case-insensitive.

### Internal

- The launcher's new `pickNewest()` choice has unit tests beside `runtimePlan()`'s, and the real bin is run with no real oam in reach. On a plain Node host, a missing `OAM_BIN` is named and `CTXLINT_RUNTIME=oam` exits 1. Posing as oam through a preloaded `process.versions.oam`, a below-floor host hands off to a Node child (or exits 1 with no Node on `PATH`), `CTXLINT_RUNTIME=node`/`NODE` hands off from a supported oam host, and a below-floor host whose chosen oam fails to spawn (a preload points the first `spawn` at a missing path) still runs the CLI through the Node fallback, with a note that the newer oam would not start. Reverting to first-found selection, letting a below-floor host run in-process, dropping the floor back to 0.9.0, letting the exit mirror act on a child that never spawned, or saying "no newer oam was found" after a found oam failed to start each turns the suite red.
...
- README: the Options block was missing `--lsp` and `--no-ignore-file`, its `Commands:` list was missing `serve`, and its usage line was missing `[command]`; the Config Reference table was missing `ignoreRules` and `hooksGlobal` (#64). `ignoreRules` was named in prose but defined nowhere outside the code, so it now has rows for its four fields -- including that `pathPattern` is honored only for `session-stale-memory` -- and the example config uses one. Tests now fail when the two drift again: `entry.test.ts` compares the flag and command names in `--help` with the Options block (`serve` is the one README-only command, since `src/index.ts` routes it before commander), and `config.test.ts` compares the table's top-level fields with `KNOWN_CONFIG_KEYS`, now exported, and loads the example config expecting no warnings.
...
- **Gradle and Maven commands are extracted and checked (#61).** The command extractor recognized a fixed tool list with no JVM build tool on it, so `./gradlew :server:test` and `./mvnw -pl flink-core-api test` were never extracted, and a JVM repository -- which rarely has a `package.json`, so every package.json-gated rule is silent there too -- linted clean by default. `./gradlew`, `gradle`, `./mvnw`, `mvn` and the Windows `gradlew.bat`/`mvnw.cmd`/`mvn.cmd` wrappers are now extracted. The tool name must be followed by whitespace or the end of the command, because `\b` also matches before `.` and `/`, and `gradle.properties` and `gradle/libs.versions.toml` are file names written in the same backticks. Two experimental rules consume the new references:
  - **`commands/gradle-project-not-found` (error).** A task path's project part (`:server` in `:server:test`) names no project of the build. The project set is read from `settings.gradle(.kts)` only when it is a closed set: literal `include`/`includeFlat` with implicit parents, literal `includeBuild` names (followed transitively, since Gradle addresses a build included by an included build from the root), literal renames, `buildSrc`, with conditionals flattened into a superset. Any construct that could add or rename an unseen project -- a non-literal include, a compound or scope-function rename, `apply from:`, a settings plugin outside a verified allowlist, a `buildscript` block, dynamic dispatch -- makes the rule silent for that build. Segments resolve the way Gradle's `NameMatcher` does (exact, case-insensitive, prefix, camelCase and kebab-case abbreviation), and only a segment matching nothing is reported; an ambiguous abbreviation is not. Skipped: commands that retarget the build (`-p`, `-c`, `-b`, `--include-build`, `-I`, in every spelling, or a `cd` earlier in the fence or prompt run), `./gradlew` in a nested build that has no wrapper of its own, relative paths outside the build root, and unmarked placeholders like `:module-name:jvmTest`.
  - **`commands/maven-module-not-found` (error).** A `-pl`/`-rf` selector matches no reactor project, by directory, POM file, `:artifactId` or `groupId:artifactId`. The reactor comes from `<modules>`/`<subprojects>` of the root POM and every profile, recursively, compared in the file system's own casing. A `${property}` in a module path, an unreadable or non-UTF-8 POM, an unbalanced element tree, or Maven 4 automatic subproject discovery (an aggregator on a model newer than 4.0.0, the version possibly implied by the namespace, with no top-level module list) makes it unknowable and the rule silent. A property inside an artifactId (`flink-dist-scala_${scala.binary.version}`) is matched as a wildcard rather than silencing every id selector in the repository. `-f` (including `-fpom.xml`), `-N`, `.mvn/maven.config` retargeting, `?`-optional selectors, placeholders (`<module>`, `%MODULE%`, `:module`, `path/to/module`), and paths that leave the linted directory are skipped.

  Measured against 25 JVM repositories with a root `AGENTS.md`, `CLAUDE.md` or `.github/copilot-instructions.md` (416 build-tool invocations): no findings, with 44 of 62 Gradle project references and all 78 Maven selectors actually evaluated. The other 18 Gradle references sit in elasticsearch, JabRef and grails-core, whose settings are not statically enumerable, and stay silent. Seeded typos in copies of kafka's and flink's docs fire with a `Did you mean` suggestion. On 69 real settings files the reader closes 23, every one also closed by an independent research classifier, with a project set containing that classifier's in every case. An adversarial review then independently reproduced 33 defects -- the Gradle ones against a real Gradle 8.14.3, the Maven ones against the Maven 3.9 and 4.0 sources -- including transitive included builds, renames that slipped the reader, Maven 4 discovery detection and attached option spellings; all are fixed here and pinned by tests. Neither tool is ever executed.

### Changed

- **`commands/exit-status-masked` can newly fire on a line that starts with a JVM command.** Such lines were not extracted before, so a masked verifier chained after one -- `./gradlew assemble && npx tsc --noEmit | tail -3 && echo "types ok"` -- was invisible. The finding is correct (the `tail` still discards `tsc`'s status), and a test pins it. A JVM command itself never triggers the rule: its verifier list has no Gradle or Maven entry and it ignores `||`, so `./gradlew test || true` stays silent. No other existing rule's output changes: a Gradle or Maven command matches none of the npm, npx, make or tool patterns, and `commands/package-json-missing` does not count one as needing a `package.json`.
...
- **PATH discovery could hand back an `oam.cmd` the launcher cannot execute, then report oam as missing.** The Windows branch walked every entry in `PATHEXT` (defaulting to `.EXE`), so a shim-style install yielded `oam.cmd` or `oam.bat` -- and Node refuses to run either through `execFile`/`spawn` without `shell: true`, with EINVAL, and for `spawn` the refusal is a synchronous throw rather than an `'error'` event. Discovery has to agree with execution, so the PATH walk is now `oam.exe` only, deliberately narrower than `PATHEXT`. A skipped shim is reported rather than ignored, because "no oam binary was found" reads as "install oam" -- the one thing that will not help someone who already has it. The new `findOamShim` (Windows only; there is no such shim concept on POSIX) looks for exactly `oam.cmd`/`oam.bat` on PATH and names what it found: under `CTXLINT_RUNTIME=oam` the error is now "no runnable oam binary was found" followed by `Found <path>, but Node cannot execute a .cmd/.bat directly.` and `Install the native oam binary, or point OAM_BIN at one.`; under the default `auto` the same note prints before the Node fallback, since silence is how someone never learns their oam install is a shape this launcher skips.
- **"Older than 0.9.0" was reported for an oam that could not be run at all.** The version gate tested `atLeast(oamVersion(oam), OAM_MIN)`, and that gate is false for two unrelated reasons: a parsed version below the floor, or a null from `oamVersion` -- which never means "old", only that the binary could not be executed or did not print a version this launcher parses (not executable, wrong arch, a `.cmd`/`.bat` Node refuses, deleted between the stat and the probe). Both landed in the same branch with the same "older than" wording, and under `CTXLINT_RUNTIME=oam` both were told to run `oam self-update` -- the one remedy that definitely does not apply to the second, sending that user after the cause it is not. The version is now read ONCE, and only when discovery found something, so the branch can tell the two apart without a second subprocess: a parsed version reports `<path> is oam <x.y.z>, older than 0.9.0` and recommends `oam self-update`, while a null reports `<path> could not be run, or did not report a version this launcher understands` and recommends checking that it is an executable oam binary for this platform. Neither is worth failing over under `auto` -- both still say so on stderr and fall back to Node -- and both still exit 1 under `CTXLINT_RUNTIME=oam`. The floor itself is unchanged at oam 0.9.0.
...
- **`hook-coverage/dead-hook` false positives on Windows / Git Bash permission entries.** Three token shapes authored in Git Bash on Windows were reported as dead hooks even though the script was fine or the token was never a path. (A) A permission matcher's trailing prefix wildcard (`Bash(bash ./scripts/gate.sh*)`, and the `:*` form) is now stripped before the existence check, so a real script referenced by a permission rule no longer reads as a missing file purely because of the `*`. (B) MSYS `//FLAG` tokens (`tasklist //FI`, `taskkill //F` -- Git Bash's escape for a native `/FLAG` argument) are recognized as command flags and skipped instead of being treated as absolute paths; a genuine UNC path (`//host/share`) is left alone. (C) MSYS drive paths (`/c/Users/...`) are translated to their Win32 form (`c:/Users/...`) on Windows before the existence check so a real file resolves; on POSIX `/c/...` is a genuine path and is never rewritten. A genuinely missing script is still reported in every case -- the wildcard strip does not suppress real dead hooks. `checkHookCoverage` gains an injectable `platform` param (defaulting to `process.platform`, mirroring the existing injectable `homeDir`) so the Windows-only drive translation is deterministically testable. New unit tests for the three helpers (`stripMatcherWildcard`, `isMsysFlag`, `translateMsysDrivePath`) plus integration cases through `checkHookCoverage`, including a win32-gated test that resolves a real `/c/` path.
- **`hook-coverage` no longer flags a mid-token glob.** A permission entry whose path contains a `*` that is not the trailing matcher wildcard (`Bash(cat ./logs/*.json)`) is a glob, not a single file, so there is nothing to stat -- it is now skipped instead of reported as a phantom missing file. Follow-up to the Windows/MSYS fix above; regression-tested.
- **`paths` false positives on directory-diagram fences and lowercase slash-prose.** Two over-extraction classes in `extractPathReferences` (`src/core/parser.ts`) that turned a clean `CLAUDE.md` into a wall of confident-but-wrong "directory does not exist" errors. (A) Inside a bare/unlabeled fence, an INDENTED line whose whole token is a lone single-segment directory name (`api/`, `db/`) is no longer emitted: in an ASCII directory diagram such a child is written relative to its parent line (`apps/` then `  api/` means `apps/api/`, not a root-level `api/`), so the root-relative token was a phantom. Non-indented entries and any multi-segment / extension-bearing token (`src/utils/helpers.ts`) are still validated, so a genuine flat file-tree in a bare fence is still checked. (B) A lowercase single-slash `word/word` token with no extension and no `./ ../ /` prefix (`unit/integration`, `skill/agent`, `content/length`) is now treated as prose and skipped, mirroring the existing `Capitalized/Capitalized` guard. To avoid false negatives, a token whose FIRST segment is a recognized top-level/source directory (`src apps packages scripts docs test lib dist infra config ...`) is still validated, so real 2-segment refs (`apps/api`, `src/utils`) and a genuinely broken `src/missing` keep being reported. Resolves 10 of the 13 false positives seen on the `typed` repo's `.claude/CLAUDE.md`. Residual (deferred at the time, now resolved in the follow-up entry below): multi-segment lowercase prose (`build/seed/probe/lint`) and relative-mention globs/placeholders (`meta/*_snapshot.json`, `meta/NNNN_snapshot.json`). New `fixtures/diagram-and-prose/` plus focused `paths.test.ts` cases, including negative controls that confirm a broken multi-segment path in a bare fence and a broken top-level ref in prose are still caught.
- **`paths` false positives from inline-code / prose extraction (follow-up; resolves the residual above and issue #41).** The remaining four over-extraction shapes on an always-loaded `.claude/CLAUDE.md`, every one naming a path that exists on disk. (A) A 3+ segment all-lowercase slash-prose token (`build/seed/probe/lint`, from "build, seed, probe, lint scripts") is now treated as prose, extending the 2-segment guard to 3+ segments. Unlike the 2-segment guard it does NOT rescue an allow-listed first segment (`build` is allow-listed, yet the whole token is prose), because a genuine 3+ segment path almost always carries an extension (`src/core/checks/paths.ts`) or a trailing slash (`src/core/checks/`) -- both of which stay validated, so a real resolvable ref is not lost. (B) A repeated-uppercase placeholder segment (`meta/NNNN_snapshot.json`, where `NNNN` stands in for a migration number) is doc shorthand, not a filename: a run of 3+ identical uppercase letters is skipped. (C) An unanchored single-`*` glob whose first segment is not a recognized top-level dir (`meta/*_snapshot.json`, shorthand for the nested `packages/db/migrations/meta/*_snapshot.json`) is prose shorthand; anchored globs (`./`, `../`, absolute, `**`, or a known top-level first segment such as `src/**/*.ts`) keep being validated. (D) An explicit-relative ref in a ROOT-LEVEL agent-config doc (`<root>/.claude/CLAUDE.md`) now resolves `./`-relative refs against the project root as a SECONDARY base (`contextDir` stays primary, so `@`-import / `./` autofix coordinates and rename provenance are unchanged), so `./packages/*` in `.claude/CLAUDE.md` means "packages/ from the repo root"; a non-dot subdir doc (`docs/CLAUDE.md`) is unaffected. Every guard ships a negative control confirming genuinely broken refs are still caught (extension-bearing multi-segment paths, anchored globs, top-level refs); new focused `paths.test.ts` cases plus an end-to-end all-four fixture. The secondary "N auto-fixable broken paths" session-start over-count was a downstream host-hook tally over these now-eliminated errors -- ctxlint's own `--fix-dry-run` correctly reported "No auto-fixable issues" throughout, so no fixer change was needed.
...
- **MAJOR** — breaking change to the public surface:
  - **Stable** rule ID removed, renamed, or moved to a different category. (An experimental one may be withdrawn without a major bump -- see the closing paragraph of this section, and the 0.9.5 retraction of `commands/npm-auth-trap` above.)
...
The experimental-rule line is a correction, not a restatement: from the 0.9.4 entry above until 0.25.0, this policy placed a NEW experimental rule under PATCH, and practice had already diverged from it more than once -- 0.16.0 shipped the experimental `mcp-security/secret-scan-skipped` under a minor bump, and 0.25.0 shipped the experimental `session/large-read` the same way. The wording here is corrected to match that practice; releases cut before 0.25.0 are not retroactively reclassified.
