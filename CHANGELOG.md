...
- The launcher's new `pickNewest()` choice has unit tests beside `runtimePlan()`'s, and the real bin is run under a preloaded `process.versions.oam` with no usable oam in reach: a missing `OAM_BIN` is named, `CTXLINT_RUNTIME=oam` exits 1, a below-floor host hands off to a Node child (or exits 1 with no Node on `PATH`), and `CTXLINT_RUNTIME=node`/`NODE` hands off from a supported oam host. Reverting to first-found selection, letting a below-floor host run in-process, or dropping the floor back to 0.9.0 each turns the suite red.
